...
pub fn create_new_room_member(conn: &mut PgConnection, rid: &i64, mid: &i64) -> Result<RoomMember, DieselError> {
    ...
    match get_room_member(conn, mid, rid) {
        Ok(rm) => Ok(rm),
        Err(_) => {
            let resp = diesel::insert_into(room_member).values(&new_room_member).get_result::<RoomMember>(conn)?;
            return Ok(resp);
        }
    }
}
...
pub fn get_room_member(conn: &mut PgConnection, mid: &i64, rid: &i64) -> Result<RoomMember, DieselError> {
    let cnv = room_member
        .filter(member_id.eq(mid).and(room_id.eq(rid)).and(deleted_at.is_null()))
        .first(conn)
        .optional()?;
    match cnv {
        Some(cnv) => Ok(cnv),
